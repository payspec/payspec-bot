 ...
 /*
  ...
  async initJSONRPCServer()
         ...
         var server = jayson.server({
           ...
           generateOffchainInvoice: async function(args, callback) {
            ...
            /* payspecData = {
              ...
              console.log('generateOffchainInvoice: ', args)
           ...
           setInvoicePaidCallbackURL: async function(args, callback)
           {
             ...
             var invoices = await self.mongoInterface.upsertOne('merchantServerInvoicePaidCallbackURL', {} , {value: callbackURL  } )
     ...
     async calculateInvoiceUUID( newInvoiceData  )
     {
       ...
       //_init is not a function
       var amountDueBN = new BigNumber(newInvoiceData.amountDue)
       //amount due is too large a number !!
       console.log(' amount due ', amountDueBN)
        console.log('calculateInvoiceUUID, sha 3 inputs ',   newInvoiceData.refNumber, newInvoiceData.description, web3utils.toChecksumAddress(newInvoiceData.tokenAddress), amountDueBN, web3utils.toChecksumAddress(newInvoiceData.recipientAddress), newInvoiceData.ethBlockExpiresAt)

      //  var digest = web3utils.soliditySha3({t: 'uint256', v: newInvoiceData.refNumber  }, {t: 'string', v: newInvoiceData.description }, {t: 'address', v: web3utils.toChecksumAddress(newInvoiceData.tokenAddress) }, {t: 'uint256', v: amountDueBN } t: 'address', v: web3utils.toChecksumAddress(newInvoiceData.recipientAddress)}, {t: 'uint256', v: newInvoiceData.ethBlockExpiresAt }, );
